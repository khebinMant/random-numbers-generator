...
export default function CongruencialCuadratico() {
  const [inputs, setInputs] = useState({ a: 3, b: 5, c: 7, m: 100, x0: 11, cantidad: 10 })
  ...
  const generar = () => {
    ...
    for (let i = 0; i < inputs.cantidad; i++) {
      x = (inputs.a * x * x + inputs.b * x + inputs.c) % inputs.m
      numeros.push(x)
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8">
      <div className="container mx-auto px-4 max-w-7xl">
        <h1 className="text-3xl font-bold text-center mb-8">Algoritmo Congruencial Cuadrático</h1>
        <p className="text-center text-gray-600 mb-8">X<sub>k+1</sub> = (a × X<sub>k</sub>² + b × X<sub>k</sub> + c) mod m</p>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-4">
              ...
              <div><label className="block text-sm font-medium mb-2">c</label><input type="number" value={inputs.c} onChange={(e) => setInputs(prev => ({ ...prev, c: parseInt(e.target.value) || 0 }))} className="w-full px-3 py-2 border rounded-md bg-gray-100" /></div>
