...
export default function MultiplicadorConstante() {
  const [inputs, setInputs] = useState({ semilla: 1234, constante: 3, cantidad: 10 })
  ...
  const generar = () => {
    ...
    const maxVal = Math.pow(10, x.toString().length)
    
    for (let i = 0; i < inputs.cantidad; i++) {
      x = (x * inputs.constante) % maxVal
      numeros.push(x)
      normalizados.push(x / maxVal)
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8">
      <div className="container mx-auto px-4 max-w-7xl">
        <h1 className="text-3xl font-bold text-center mb-8">Algoritmo Multiplicador Constante</h1>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-gray-200 rounded-lg p-6">
            <h2 className="text-xl font-semibold mb-4">Parámetros</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-2">Semilla</label>
                <input type="number" value={inputs.semilla} onChange={(e) => setInputs(prev => ({ ...prev, semilla: parseInt(e.target.value) || 0 }))} className="w-full px-3 py-2 border rounded-md bg-gray-100" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">Constante</label>
                <input type="number" value={inputs.constante} onChange={(e) => setInputs(prev => ({ ...prev, constante: parseInt(e.target.value) || 1 }))} className="w-full px-3 py-2 border rounded-md bg-gray-100" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">Cantidad</label>
                <input type="number" value={inputs.cantidad} onChange={(e) => setInputs(prev => ({ ...prev, cantidad: parseInt(e.target.value) || 1 }))} className="w-full px-3 py-2 border rounded-md bg-gray-100" />
              </div>
            </div>
