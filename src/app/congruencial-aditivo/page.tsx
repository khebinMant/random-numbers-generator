...
export default function CongruencialAditivo() {
  const [inputs, setInputs] = useState({ m: 100, semillas: [23, 45, 12, 67], cantidad: 10 })
  ...
  const generar = () => {
    const numeros: number[] = [...inputs.semillas]
    const normalizados: number[] = inputs.semillas.map(s => s / inputs.m)
    
    for (let i = inputs.semillas.length; i < inputs.cantidad; i++) {
      const x = (numeros[i - 1] + numeros[i - inputs.semillas.length]) % inputs.m
      numeros.push(x)
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8">
      <div className="container mx-auto px-4 max-w-7xl">
        <h1 className="text-3xl font-bold text-center mb-8">Algoritmo Congruencial Aditivo</h1>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-4">
              ...
              <p className="text-xs text-gray-600">Semillas iniciales: {inputs.semillas.join(', ')}</p>
            </div>
